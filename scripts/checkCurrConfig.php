...
function createdbTableAssocArr($dbh, $dbTable){
	$outArr=array();
...
if( ! $simulation_mode) {
	...
	if( ! ( count( arrayRecursiveDiff( createTermoServerAssocArray($termoServerINI) , createdbTableAssocArr($dbh, "zernoib.debug_sensors")   ) )==0 &&
			...
			debug_drop_all_tables($dbh);
			debug_create_silo_table($dbh, $termoServerINI);
			debug_create_sensors_table($dbh, $termoServerINI);
	}
	//	Считывание парметров температур, скоростей и уровней в виде, аналогичном тому, который видает термосервер
	$arrayOfTemperatures = debug_update_temperature_values($dbh);
	$arrayOfTempSpeeds   = debug_update_temperature_speeds_values($dbh);
	$arrayOfLevels       = debug_update_level_values($dbh);
