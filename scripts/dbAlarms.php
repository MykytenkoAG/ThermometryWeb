...
function resetACK(){
	...
	$sql_joinedTable = "SELECT s.sensor_id, s.current_temperature, e.error_description
							FROM sensors AS s
								JOIN prodtypesbysilo AS pbs
									ON s.silo_id=pbs.silo_id
								JOIN prodtypes AS pr
	...
	$cond_RST_sens_errors="WHERE (s.NACK_err=0 AND s.ACK_err=1 AND (s.is_enabled=0 OR current_temperature < 840))";		// неисправные датчики
